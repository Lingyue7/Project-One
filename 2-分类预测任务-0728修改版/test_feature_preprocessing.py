...
class PotentialFeaturePreprocessorTests(unittest.TestCase):
    def test_dual_label_cohort_matches_the_optimization_customer_pool(self):
        raw = pd.DataFrame({
            "cst_id": ["a", "b", "c", "d"],
            "loanacctno": ["l1", "l2", "l3", "l4"],
            "rt_acct_eff_date_1": [
                "2025-01-01", "2025-02-01", "2025-03-01", "2025-04-01"
            ],
            "rt_acct_stat_2": ["1", "1", "1", "1"],
            "rt_acct_stat_2_end": ["1", "3", None, "1"],
            "ac_curr_bal_diff": [1.0, 10.0, 5.0, np.nan],
            "ba_out_bal_diff": [1.0, 10.0, 5.0, 2.0],
            "credamt": [100.0, 200.0, 300.0, 400.0],
        })

        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "dual_cohort.csv"
            raw.to_csv(path, index=False, encoding="utf-8-sig")
            result = load_kechuang_potential(
                str(path),
                target="y_freq",
                apply_maturity_filter=False,
                apply_eff_date_filter=False,
                exclude_dq_start_customers=True,
                label_threshold_train_ratio=0.5,
        ...
        cleaned = result[0]
        self.assertEqual(cleaned["cst_id"].tolist(), ["a", "b"])
        self.assertIn("y_freq", cleaned.columns)
        self.assertIn("y_dq_risk", cleaned.columns)
    ...
    def test_explicit_numeric_category_is_encoded_as_nominal(self):
        train = pd.DataFrame({
            "y_freq": [0, 1, 0],
            "custom_code": [10.0, 20.0, np.nan],
        })
        validation = pd.DataFrame({
            "y_freq": [0],
            "custom_code": [30.0],
        ...
        preprocessor = PotentialFeaturePreprocessor(
            categorical_features=["custom_code"]
        ).fit(train)
        transformed = preprocessor.transform(validation)

        self.assertIn("custom_code", preprocessor.categorical_features_)
        self.assertNotIn("custom_code", preprocessor.numeric_features_)
        self.assertEqual(transformed.loc[0, "custom_code"], -1)

    def test_age_and_tier_cannot_be_forced_to_nominal_categories(self):
        with self.assertRaisesRegex(ValueError, "连续变量"):
            PotentialFeaturePreprocessor(categorical_features=["age"])
        with self.assertRaisesRegex(ValueError, "有序变量"):
            PotentialFeaturePreprocessor(categorical_features=["档位"])
