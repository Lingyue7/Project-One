...
科创潜在客户数据预处理模块
==============================
适配新的消贷客户 SAS 取数结构，与原流程对齐。

字段变化说明：
  - 客户维表字段 brth_dt、eddgr_cd、ocp_cd 分别标准化为出生日期、学历代码、职业代码
  - 支持两种因变量（通过 target 参数切换）：
      y_freq    : 频率活跃度标签（多种构造模式，详见 build_labels_potential）
      y_dq_risk : 违约风险标签（SEQUOVERDAY > 30 → 1，否则 → 0）
  - 一客多贷时 SEQUOVERDAY 取最大值；标签构造后删除该字段，避免泄露。
...
泄露字段（drop_post_label_cols 中删除）：
  ...
  sequoverday（y_dq_risk 构造后删除；y_freq 模式在 Part1 删除）
...
def _parse_birth_date(values: pd.Series) -> pd.Series:
    """解析 DIM_IDV_CST_H 的出生日期，优先适配 08Sep2026 这类 SAS 日期文本。"""
...
def _age_on_snapshot(birth_date: pd.Series, snapshot_date: str) -> pd.Series:
    """按完整生日计算截至快照日的年龄；出生日期晚于快照日时保留缺失。"""
    snap = pd.Timestamp(snapshot_date)
    age = snap.year - birth_date.dt.year - (
        (birth_date.dt.month > snap.month)
        | ((birth_date.dt.month == snap.month) & (birth_date.dt.day > snap.day))
    ).astype("Int64")
    age = pd.to_numeric(age, errors="coerce")
    return age.mask(age < 0)
...
def deduplicate_cst_loan_keep_first(df: pd.DataFrame) -> pd.DataFrame:
    """按 (cst_id, loanacctno) 去重，按输入文件顺序保留第一条记录。"""
    loan_col = next(
        (c for c in ["loanacctno", "loan_acct_no", "loanacct_no"] if c in df.columns),
        None,
    )
    ...
    return df.drop_duplicates(subset=["cst_id", loan_col], keep="first").copy()
...
def rename_kechuang_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rename_map = {
        ...
        # 新对私客户维表字段：标准化为下游模型既有字段名。
        "eddgr_cd":            "education_cd",
        "ocp_cd":              "occup_cd",
        "cur_aum":           "当前aum",
        "cur_lum":           "当前lum",
        "tech_talent_score": "科技人才对应得分",
...
def aggregate_by_customer_potential(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    字段说明：
      ...
      - sequoverday：取最大连续拖欠天数，用于构造客户级违约标签
    ...
    # 客户任一贷款连续拖欠天数大于 30 天即视为违约，因此取最大值。
    loan_special_agg = {"sequoverday": "max"}
...
def clean_data_potential(
    ...
    清洗 Part1（在因变量构造之前执行）：
      - cst_id 转字符串
      - 将 brth_dt 解析为出生日期，并按 snapshot_date 构造年龄
      - 删除所有 ID / 日期 / 无建模意义字段（含新取数结构新增的字段）
      - y_freq 模式：直接删除 sequoverday（不需要此字段）
      - y_dq_risk 模式：保留 sequoverday（供 build_labels_potential 使用）
    ...
    # ── 出生日期转年龄（快照日完整年龄，不用近似天数除法） ──
    if "brth_dt" in df.columns:
        birth_date = _parse_birth_date(df["brth_dt"])
        df["age"] = _age_on_snapshot(birth_date, snapshot_date)
        print(f"  已由 brth_dt 生成 age：有效 {int(df['age'].notna().sum()):,} 人")
    ...
    pre_drop_cols = [
        ...
        "bank_cust_become_date",  # 兼容旧数据；新数据无此字段
        "brth_dt",
        "acctbegindate",
    ...
    # y_freq 模式下 sequoverday 无用，在 Part1 直接删除。
    if target == "y_freq":
        pre_drop_cols.append("sequoverday")
...
def drop_post_label_cols(df: pd.DataFrame, target: str = "y_freq") -> pd.DataFrame:
    ...
    删除以下字段：
      ...
      - y_dq_risk 模式额外删除 sequoverday（构造标签后防泄露）
    """
    df = df.copy()
    ...
    # y_dq_risk 模式：构造标签后删除 sequoverday（防泄露）
    if target == "y_dq_risk":
        post_drop_cols.append("sequoverday")
...
def build_labels_potential(
    ...
    target='y_dq_risk'：
      sequoverday（聚合后最大连续拖欠天数）> 30 → 1，否则 → 0
    """
    work = df.copy()
    n_total = len(work)
    ...
    # ════════════════════════════════════════
    if target == "y_dq_risk":
        if "sequoverday" not in work.columns:
            raise ValueError("y_dq_risk 模式需要字段 sequoverday，请确认 SQL 已提取 SEQUOVERDAY。")
        overdue_days = pd.to_numeric(work["sequoverday"], errors="coerce")
        valid_overdue = overdue_days.notna() & np.isfinite(overdue_days)
        n_missing_overdue = int((~valid_overdue).sum())
        if n_missing_overdue:
            print(
                f"  [警告] sequoverday 缺失/无效 {n_missing_overdue:,} 人，"
                "标签无法定义，已从建模样本排除。"
            )
            work = work.loc[valid_overdue].copy()
            overdue_days = overdue_days.loc[valid_overdue]
        if work.empty:
            raise ValueError("sequoverday 没有任何有效值，无法构造 y_dq_risk。")
        n_total = len(work)
        work["y_dq_risk"] = (overdue_days > 30).astype(int)
        n_pos = int(work["y_dq_risk"].sum())
        print(f"\n  {'-'*55}")
        print(f"  y_dq_risk 构造统计")
        print(f"  {'-'*55}")
        print(f"  sequoverday > 30 -> 1 : {n_pos:>6,} 人  ({n_pos/n_total:.2%})")
        print(f"  {'-'*55}")
...
class PotentialFeaturePreprocessor:
    ...
    _DROP_COLS = {
        ...
        "y_freq", "y_dq_risk", "sequoverday", "split_eff_date",
    }
    _TIER_ORDER = {
        "F3": 1, "F3级": 1, "f3": 1,
        "F2": 2, "F2级": 2, "f2": 2,
...
def load_kechuang_potential(
    ...
    dedup_cst_loan         : 旧版兼容开关；True 等同于 dedup_cst_loan_mode="exact"
    dedup_cst_loan_mode    : "none"=仅统计；"exact"=仅删完全相同副本，冲突报错；
                             "cst_loan_first"=按 cst_id、loanacctno 保留输入的第一条
    ...
    def _count_dq_rows(df, label):
        """行级数据：统计连续拖欠天数大于 30 天的贷款和客户数。"""
        if target != "y_dq_risk" or "sequoverday" not in df.columns:
            return
        days = pd.to_numeric(df["sequoverday"], errors="coerce")
        dq_mask  = days > 30
        dq_rows  = int(dq_mask.sum())
        dq_cst   = int(df.loc[dq_mask, "cst_id"].nunique()) if "cst_id" in df.columns else -1
        null_cnt = int(days.isna().sum())
        total_rows = len(df)
        total_cst  = df["cst_id"].nunique() if "cst_id" in df.columns else -1
        print(f"  +-- [违约追踪] {label}")
        print(f"  |  总行数={total_rows:,}  总客户={total_cst:,}")
        print(f"  |  连续拖欠天数>30的行={dq_rows:,}  对应客户数={dq_cst:,}")
        print(f"  +-- SEQUOVERDAY为空/无效={null_cnt:,}")

    def _count_dq_agg(df, label):
        """聚合后数据（一客一行）：统计连续拖欠天数大于 30 天的客户数。"""
        if target != "y_dq_risk" or "sequoverday" not in df.columns:
            return
        days = pd.to_numeric(df["sequoverday"], errors="coerce")
        dq_cst   = int((days > 30).sum())
        null_cnt = int(days.isna().sum())
        total    = len(df)
        print(f"  +-- [违约追踪] {label}")
        print(f"  |  总客户={total:,}")
        print(f"  |  连续拖欠天数>30的客户={dq_cst:,}  ({dq_cst/total:.2%})")
        print(f"  +-- SEQUOVERDAY为空/无效={null_cnt:,}")
    # ─────────────────────────────────────────
    ...
    valid_dedup_modes = ("none", "exact", "cst_loan_first")
    if dedup_cst_loan_mode not in valid_dedup_modes:
        raise ValueError(
            "dedup_cst_loan_mode 仅支持 none / exact / cst_loan_first"
        )
    # 兼容旧版 True 开关：未指定新模式时仍执行原有的严格去重。
    if dedup_cst_loan and dedup_cst_loan_mode == "none":
        dedup_cst_loan_mode = "exact"

    print(
        f"[2.5/8] (cst_id, loanacctno) 重复检查（mode={dedup_cst_loan_mode}）..."
    )
    df0 = report_cst_loan_duplicates(df0)
    if dedup_cst_loan_mode == "exact":
        n_before = len(df0)
    ...
    elif dedup_cst_loan_mode == "cst_loan_first":
        n_before = len(df0)
        df0 = deduplicate_cst_loan_keep_first(df0)
        print(
            f"  已按 cst_id + loanacctno 保留第一条：{n_before:,} → {len(df0):,} 行，"
            f"删除 {n_before - len(df0):,} 行"
        )
    ...
    if exclude_dq_start_customers:
        print("[3.7/8] 新取数不含起点账户状态，跳过起点已违约客户剔除。")
